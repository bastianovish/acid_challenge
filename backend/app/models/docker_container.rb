...
class DockerContainer
  ...
  def memory_usage
    '%.2f MiB' % ((stats['memory_stats']['usage'].to_f -  stats['memory_stats']['stats']['cache'].to_f)/ (1024 * 1024))
  end

  def memory_limit
    '%.3f GiB' % (stats['memory_stats']['limit'].to_f / (1024 * 1024 * 1024))
  end

  def network_i
    '%.1f kB' % (stats['network']['rx_bytes'].to_f / 1024)
  end

  def network_o
    '%.2f kB' % (stats['network']['tx_bytes'].to_f / 1024)
  end
